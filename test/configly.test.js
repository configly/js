...
test('Overriding the host works', () => {
  ...
  const matcher = expect.stringMatching(new RegExp(`^${host}`))
...
test('Default URL is expected', () => {
  ...
  const matcher = expect.stringMatching(new RegExp(`^${EXPECTED_URL}`))
...
test('get() called with an unknown key', async (done) => {
  ...
  const v = await Configly.init('abc').get(key)
...
test('Caching respects TTL correctly', async done => {
  ...
  // Set to after expiry
  const newTimeNow = timeNow + SLOGAN_TTL + 1
  jest.spyOn(Date, 'now')
...
test('Caching respects TTL correctly with several values', async done => {
  ...
  // Set to after expiry
  const newTimeNow = timeNow + CITIES_TTL + 1
  jest.spyOn(Date, 'now')
    .mockReturnValue(1000 * newTimeNow)

  const newTtl = 400
  const newVal = 'boston <3'
  const newResp = {
    data: {
  ...
  const newTimeAgain = timeNow + SLOGAN_TTL + 50
  jest.spyOn(Date, 'now')
